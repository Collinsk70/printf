              ...
              int length, char paddingChar, char extraCharacter)
{
    ...
    if (width > length)
    {
        ...
        if (flags & F_MINUS && paddingChar == ' ') /* Assign extra buffer */
        {
            ...
            return (write(1, &buffer[index], length) + write(1, &buffer[1], charIndex - 1));
        }
        else if (!(flags & F_MINUS) && paddingChar == ' ') /* Extra character to buffer */
        {
            if (extraCharacter)
                buffer[--index] = extraCharacter;
            return (write(1, &buffer[1], charIndex - 1) + write(1, &buffer[index], length));
        }
        else if (!(flags & F_MINUS) && paddingChar == '0') /* Extrapadding */
        {
            if (extraCharacter)
                  ...
                  char paddingChar, char extraCharacter, int paddingStart)
{
    ...
    if (width > length)
    {
        ...
        if (flags & F_MINUS && paddingChar == ' ') /* Assign extra*/
        {
            ...
            if (extraCharacter)
                buffer[--index] = extraCharacter;
            return (write(1, &buffer[index], length) + write(1, &buffer[3], charIndex - 3));
        }
        else if (!(flags & F_MINUS) && paddingChar == ' ') /* Extra character*/
        {
            ...
            if (extraCharacter)
                buffer[--index] = extraCharacter;
            return (write(1, &buffer[3], charIndex - 3) + write(1, &buffer[index], length));
        }
